...
import { installPermissionHandlers } from './permissionHandler.js';
import { installHeaderFix } from './cspHeaderHandler.js';
import { getWindowDefaults } from './windowUtils.js';
import { logger } from './logger.js';
import { asUnsafe, asType } from '../../shared/typeUtils.js';
...
export class AccountViewManager implements IAccountWindowManager {
  ...
  saveAccountWindowState(accountIndex: AccountIndex): void {
    ...
    const bounds = this.hostWindow.getBounds();
    const state: AccountWindowState = {
      ...
      isMaximized: this.hostWindow.isMaximized(),
    };
    const all: AccountWindowsMap =
      asType<AccountWindowsMap | undefined>(configGet('accountWindows')) ?? {};
    const next: AccountWindowsMap = { ...all, [accountIndex]: state };
    try {
      configSet('accountWindows', next);
    } catch (error: unknown) {
      log.warn('[AccountViewManager] saveAccountWindowState failed:', error);
    }
  }
