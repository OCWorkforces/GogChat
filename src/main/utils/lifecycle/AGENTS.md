...
| `performanceMonitor.ts` | ~90 | Startup timing markers (`PHASE_START`/`PHASE_END`), memory snapshots, per-renderer sampling |
| `performanceExport.ts` | ~30 | Formats `performance-metrics.json` for CI perf budget gate (`scripts/check-perf-budget.js`) |
| `performanceTypes.ts` | ~15 | Typed interfaces for timing markers, memory readings, performance budgets |
